/**
 * This file is derived from the fatfs lpc17xx example,
 * I modified it to suit the stm32f1 and the libopencm3 library.
 * 
 * I left ChaN original copyright below.
 *
 * I am sure this could be optimized with 16 bit reaeds and write
 * or by using DMA.
 */
...
#define	INS			1 /* Socket status always true, olimexino-stm32 has no way to check */
#define	WP			0 /* Card write protection (true:yes, false:no) */

#define	CS_LOW()	{ gpio_clear(GPIOD, GPIO2); }
#define	CS_HIGH()	{ gpio_set(GPIOD, GPIO2); }

#define FCLK_FAST() { spi_set_baudrate_prescaler(SPI2, SPI_CR1_BR_FPCLK_DIV_64); }
#define FCLK_SLOW() { spi_set_baudrate_prescaler(SPI2, SPI_CR1_BR_FPCLK_DIV_256); }
...
void power_on (void)	/* Enable SSP module and attach it to I/O pads */
{
	...
	/* CS Setup GPIO 2 port D for Cable Select. */
